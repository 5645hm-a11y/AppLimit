$filePath = "c:\Users\HM\Documents\AppLimit\AppLimit\app\src\main\java\com\example\safetimeguard\LanguageManager.kt"

$content = Get-Content $filePath -Raw -Encoding UTF8

# Find the location to insert - right before the final closing of the map
# Look for the pattern:  UKRAINIAN to "П'ятниця: додаткові правила блокування активні"
#                            )
#            )
...
# Replace the pattern: last ) with new translations + )
$newContent = $content -replace '(\s+UKRAINIAN to "[^"]*"\s+)\)', "`${1}$translations`n            )"

Set-Content -Path $filePath -Value $newContent -Encoding UTF8
Write-Host "Successfully added missing translation keys"
